#!/bin/bash
 ...
 det_py(){
 	...
 	rm /etc/Y/PortPD.log &>/dev/null
 	rm -rf /etc/Y/protocols/lacasitamx.py
 ...
 tput cuu1 && tput dl1
     ...
     echo -e "$BARRA"
     [[ ! -e /etc/Y/protocols/lacasitamx.py ]] && rm -rf /etc/VPS-AGN/protocols/lacasitamx.py
 #
 	# Initializing or Proxy
 	(
 	less << CPM > /etc/Y/protocols/lacasitamx.py
 # -*- coding: utf-8 -*-
...
CPM
 ...
 chmod +x /etc/Y/protocols/lacasitamx.py &>/dev/nulll
